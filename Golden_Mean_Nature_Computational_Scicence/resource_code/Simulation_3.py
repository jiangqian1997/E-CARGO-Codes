...
def generateContinuous_Q_random_alpha(start_time, end_time, expression_part):
	...
	if end_time > start_time:
		if agent_type == 0: # 佛系人格
			# Q_latest_val = epsilon*np.sin(omega*t)+beta*(-np.e**(-t)+1)+Q_val
			Q_latest_val =  beta*end_time - beta*start_time
		elif agent_type == 1: # 自卑型人格
			# alpha*np.sin(4*t-np.pi/2) -> 情绪牵制部分
			tmp_alpha = -alpha # 保证Q_latest_val > 0
			Q_latest_val = (-tmp_alpha*np.cos(4*end_time-np.pi/2)/4 + (beta+tmp_alpha)*end_time) - (-tmp_alpha*np.cos(4*start_time-np.pi/2)/4 + (beta+tmp_alpha)*start_time)
		elif agent_type == 2: # 越挫越勇型人格
			# e.g., 1/2*sin(x-pi/2)+1/2
			tmp_alpha = alpha
			# Q_latest_val = tmp_alpha*np.sin(t-np.pi/2) + tmp_alpha + beta*(-np.e**(-t)
...
# 生成不同比例数的代理
def generateRatioAgent(c_1_ratio, c_2_ratio, agent_number):
	base_number_maximum = agent_number//(c_1_ratio+c_2_ratio)
	# base_number = random.randint(1, base_number_maximum)
	base_number = base_number_maximum
	c_1 = base_number*c_1_ratio
	c_2 = base_number*c_2_ratio
	c_0 = agent_number-c_1-c_2
	return [c_0, c_1, c_2]
...
if __name__ == '__main__':
	...
	# record_agent_type_list = [[92, 10, 100]]
	record_agent_type_list = [[1, 10], [1, 8], [1, 4], [1, 2], [1, 1], [2, 1], [4, 1], [8, 1], [10, 1]]
	for c_1_ratio, c_2_ratio in record_agent_type_list:
		agent_type_list = generateRatioAgent(c_1_ratio, c_2_ratio, agent_number)
		# 生成随机的角色
		role_list = []
		fileName = "simulation3_final.txt"
		for j in range(role_number):
			role_id = j
		...
		for time in range(cyclingTimes):
			...
			orig_T_Matrix, orig_result, orig_performance, orig_L = Assignment.PuLP_GRA(Q)
			...
			for current_time in time_list:
				...
				T_Matrix, result, performance, L = Assignment.PuLP_GRA(Q_latest)
				L = np.array(L)
			...
			# print("The increased agent list----------")
			# print("The increased agent list amount is: ", str(len(increase_list)))
			# for item in increase_list:
			# 	print(item)
			# print("")
			# print("The unchanged agent list----------")
			# print("The unchanged agent list amount is: ", str(len(unchanged_list)))
			# for item in unchanged_list:
			# 	print(item)
			# print("")
			# print("The decreased agent list----------")
			# print("The decreased agent list amount is: ", str(len(decrease_list)))
			...
			# 记录到xlsx表格中
			# record_data(increase_list, 0)
			# record_data(unchanged_list, 1)
			# record_data(decrease_list, 2)
			# 记录到数据txt文件中
			# increased_name = 'increased_agents.txt'
			# unchanged_name = 'unchanged_agents.txt'
			# decreased_name = 'decreased_agents.txt'
			# recordResult(increased_name, increase_list)
			# recordResult(unchanged_name, unchanged_list)
			# recordResult(decreased_name, decrease_list)
