...
class Assignment:
	...
	@classmethod
	def PuLP_GRA(cls, Q, lambda_val = 0):
		"""
		: RQ 表示每个班级的平均成绩:
		"""
		row = len(Q)
...
# 更新情绪牵制率 alpha, alpha表示比自己优秀的人的数量比例
def updateAlphaList(qualified_vector, solution_dics):
	'''
	>>> Qian Jiang 20211207
	>>> --- input
...
def generateRandomAlphaList(agent_number):
	alpha_list = []
...
def generateAgentList(agent_type_list, agent_number):
	...
	type_list = [0]*int(agent_type_0) + [1]*int(agent_type_1) + [2]*int(agent_type_2)
...
# def generateAgentList_fixed(type_list, , agent_number):
...
def generateAgentTypeList(agent_number, ratio_type):
	'''
	>>> Qian Jiang 20220112
	>>> --- input
	`agent_number`: int, 表示agent的列表
	`ratio_type`: int, 表示哪类性格的人数占主导，# -1 C[1]/C[2] < 0; 0 C[1]/C[2] = 0; 1 C[1]/C[2] > 0
	>>> --- output
	`agent_type_list`: 1-D list, [1, 1, 1] -> 佛系人数1， 脆弱型人数1以及内卷型人数1
	'''
	epsilon = 1e-3
	min_ratio = 1/(agent_number-1)
	max_ratio = agent_number-1
	generated_random_ratio = round(random.uniform(epsilon+min_ratio, 1-epsilon), 2)
	if ratio_type == -1:
		c_2 = int(agent_number/(1+generated_random_ratio))
		c_1 = agent_number-c_2
	elif ratio_type == 0:
		agent_type_list = [0, agent_number/2, agent_number/2]
		return agent_type_list
	else:
		c_1 = int(agent_number/(1+generated_random_ratio))
		c_2 = agent_number-c_1
	agent_type_list = [0, c_1, c_2]
	return agent_type_list

# 找到班级的学生的索引的集合
def getClass(solution_dics, role_number=4):
	res = {}
	for j in range(role_number):
		class_i_idxs = [item[0] for item in solution_dics.items() if item[1] == j]
		res[j] = set(class_i_idxs)
	return res

# 判断班级是否变化
def checkUnchangedClass(orig_solution_dics, latest_solution_dics, agent_idx, role_number=4):
	orig_class_idx = orig_solution_dics[agent_idx]
	latest_class_idx = latest_solution_dics[agent_idx]
	orig_class = getClass(orig_solution_dics, role_number)
	latest_class = getClass(latest_solution_dics, role_number)
	return orig_class[orig_class_idx] == latest_class[latest_class_idx]
...
if __name__ == '__main__':
	...
	cyclingTimes = 100 # 大规模实验的循环次数
	b_val = 0.001
	time_list = [k for k in np.arange(time_interval, end_time + time_interval, time_interval)]
	# lambda_list = [-100, -10, -1, -0.1, -0.01, -0.001, -0.0001, 0, 0.0001, 0.001, 0.01, 0.1, 1, 10, 100]
	lambda_list = [-50, -10, -1, -0.1, -0.01, -0.001, -0.0001, 0, 0.0001, 0.001, 0.01, 0.1, 1, 10, 50]
	# lambda_list = [-0.001]
	# lambda_list = sorted(lambda_list, reverse=False)
	ratio_type = -1 # -1 C[1]/C[2] < 0; 0 C[1]/C[2] = 0; 1 C[1]/C[2] > 0
	# 生成随机的角色
	orig_total_performance_list = []
	latest_total_performance_list = []
	record = []
	for time in range(cyclingTimes):
		agent_type_list = generateAgentTypeList(agent_number, ratio_type)
		initial_agent_list = generateAgentList(agent_type_list, agent_number)
		role_list = []
		fileName = "simulation4_1.txt"
		agent_list = []
		for j in range(role_number):
			role_id = j
		...
		for lambda_val in lambda_list:
			agent_list = cp.deepcopy(initial_agent_list)
			flag = [0]*agent_number# 用于第一次的适应
			Q = genQMatrix(agent_list, role_list)
			orig_qualified_vector = [Q[i][0] for i in range(len(Q))]
			# beta不受Q的限制，但是alpha受Q的限制
			beta_list = generateBetaList(orig_qualified_vector, agent_number, b_val)
			orig_T_Matrix, orig_result, orig_performance, orig_L = Assignment.PuLP_GRA(Q, lambda_val)
			orig_solution_dics = getSolutionUnits(orig_T_Matrix)
			latest_solution_dics = cp.deepcopy(orig_solution_dics)
			alpha_list = generateRandomAlphaList(agent_number)
			# alpha_list = updateAlphaList(orig_qualified_vector, orig_solution_dics)
			orig_L = np.array(orig_L)
			initial_roles_performance = (orig_T_Matrix*Q).sum(axis = 0)/orig_L
			orig_alpha_list = [0]*agent_number
			Q = np.array(Q)
			...
			for current_time in time_list:
				for i in range(len(Q_latest)):
					for j in range(1):
						if flag[i] == 0:
							...
							if checkUnchangedClass(former_solution_dics,  latest_solution_dics, i):
								Q_latest[i][j] += generateContinuous_Q_random_alpha(current_time-time_interval, current_time, (agent_list[i].characteristic_type, orig_alpha_list[i], beta_list[i], Q[i][j]))
							else:
								Q_latest[i][j] += generateContinuous_Q_random_alpha(current_time-time_interval, current_time, (agent_list[i].characteristic_type, alpha_list[i], beta_list[i], Q[i][j]))

					Q_latest[i] = [Q_latest[i][0]]*len(Q_latest[0])
					orig_Q_assignment[i] = [orig_Q_assignment[i][0]]*len(orig_Q_assignment[0])
				# 更新agent的Q值
				...
				former_solution_dics = cp.deepcopy(latest_solution_dics)
				T_Matrix, result, performance, L = Assignment.PuLP_GRA(Q_latest, lambda_val)
				L = np.array(L)
				latest_solution_dics = getSolutionUnits(T_Matrix)
				# 更新当前时间的alpha_list
				# alpha_list = updateAlphaList(qualified_vector_latest, solution_dics)
				# orig_alpha_list = updateAlphaList(orig_qualified_vector, orig_solution_dics)
				cplex_result = sum((Q_latest*T_Matrix).sum(axis = 0))
				orig_cplex_result = sum((orig_T_Matrix*orig_Q_assignment).sum(axis = 0))
			cplex_result_list.append(cplex_result)
			orig_cplex_result_list.append(orig_cplex_result)
		orig_total_performance_list.append(orig_cplex_result_list)
		latest_total_performance_list.append(cplex_result_list)
	print(latest_total_performance_list)
	record.append("+++++++++++++"+str(cyclingTimes)+"---------")
	record.append("------One-time Assignment Result------")
	orig_total_performance_list = np.array(orig_total_performance_list)
	record.append(str((orig_total_performance_list).sum(axis = 0)/cyclingTimes))
	...
	latest_total_performance_list = np.array(latest_total_performance_list)
	record.append(str((latest_total_performance_list).sum(axis = 0)/cyclingTimes))
	record.append("+++++++++++++"+str(ratio_type)+"---------")
